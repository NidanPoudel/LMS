...
int info_update_user_add()
{
    info new_info = info_get();
    new_info.curr_user_id++;
...
int user_add_to_file(user *user_ptr,const char *filename)
{
    FILE *fp = fopen(filename, "a");
    if(!fp)  return 1;
    fwrite(user_ptr, sizeof(user), 1, fp);
    fclose(fp);
    return 0;
}